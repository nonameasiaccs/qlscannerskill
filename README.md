...
Give it a GraphQL URL — it runs the full scan and delivers one complete JSON report. No confirmation needed.
...
No environment setup is required. The scanner runs as an OpenClaw agent skill.

> **Note:** All scans use ethical identifying headers (`x-info`, `x-email`, `User-Agent`) on every request to identify traffic as legitimate security research.
